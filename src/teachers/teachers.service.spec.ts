...
describe('TeachersService', () => {
  ...
  describe('create', () => {
    ...
    it('creates a User, TEACHER Membership and Teacher inside a transaction', async () => {
      membershipsRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder({ organizationId: 'org-1' }));
      ...
      dataSource.transaction.mockImplementation(async (cb: (m: never) => Promise<unknown>) => cb(manager));

      const result = await service.create('actor-1', dto);

      expect(dataSource.transaction).toHaveBeenCalled();
      const createdUser = manager.create.mock.calls.find(([e]) => e === User)?.[1];
      const createdMembership = manager.create.mock.calls.find(([e]) => e === Membership)?.[1];
      const createdTeacher = manager.create.mock.calls.find(([e]) => e === Teacher)?.[1];

      expect(createdUser).toMatchObject({ email: 'nguyena@gmail.com', fullName: 'Nguyen Van A' });
    ...
    it('throws NotFoundException when a branch does not belong to the organization', async () => {
      ...
      await expect(service.create('actor-1', dto)).rejects.toThrow(NotFoundException);
    });

    it('throws ConflictException when the teacher code is already used in the organization', async () => {
      membershipsRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder({ organizationId: 'org-1' }));
      membershipsRepo.findOne.mockResolvedValue(adminMembership);
      teachersRepo.findOneBy.mockResolvedValue({ id: 't-1', teacherCode: 'GV001' });

      await expect(service.create('actor-1', dto)).rejects.toThrow(ConflictException);
    });
    ...
    it('throws ConflictException when the email already exists', async () => {
      ...
      membershipsRepo.findOne.mockResolvedValue(adminMembership);
      teachersRepo.findOneBy.mockResolvedValue(null);
      usersRepo.findOneBy.mockResolvedValue({ id: 'u-1', email: 'nguyena@gmail.com' });
    ...
    it('throws ForbiddenException when the actor is not owner/admin', async () => {
      ...
      await expect(service.create('actor-1', dto)).rejects.toThrow(ForbiddenException);
    });

    it('throws ForbiddenException when the user has no active membership in the organization', async () => {
      membershipsRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(null));
  ...
  describe('findAll', () => {
    ...
    it('throws ForbiddenException when called by a teacher', async () => {
      membershipsRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder({ organizationId: 'org-1' }));
      membershipsRepo.findOne.mockResolvedValue({ ...adminMembership, role: { name: 'Teacher' } });

      await expect(service.findAll('actor-1')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('findOne', () => {
    it('returns the teacher for an admin', async () => {
      membershipsRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder({ organizationId: 'org-1' }));
      membershipsRepo.findOne.mockResolvedValue(adminMembership);
      teachersRepo.findOne.mockResolvedValue({ id: 't-1', organizationId: 'org-1', teacherCode: 'GV001' });
    ...
    it('throws NotFoundException when the teacher does not exist', async () => {
      ...
      await expect(service.findOne('actor-1', 'missing')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findMe', () => {
    it('returns the teacher profile owned by the current user', async () => {
      membershipsRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder({ organizationId: 'org-1' }));
      teachersRepo.findOne.mockResolvedValue({ id: 't-1', userId: 'user-1', organizationId: 'org-1' });
  ...
  describe('update', () => {
    ...
    it('throws ConflictException when changing to an existing teacherCode', async () => {
      ...
      teachersRepo.findOneBy.mockResolvedValue({ id: 't-2', teacherCode: 'GV002' });
